...
def plot_AUROC(labels:np.array, predictions:np.array, figsize:tuple, style:str = None, n_boot_iters:float or int=5000, alpha:float=0.05, n_jobs:int=1):
    ...
    if style == 'science':
        ...
        ax.fill_between(interp_fpr, lower_ci_tpr, upper_ci_tpr, color="dodgerblue", alpha=0.3, label=r"%d%s CI: [%0.3f, %0.3f]" % (int((1-alpha)*100),'\%', lower_ci, upper_ci))
    else:
...
def plot_AUCPR(labels:np.array, predictions:np.array, figsize:tuple, style:str, n_boot_iters:float or int=5000, alpha:float=0.05, n_jobs:int=1):
    ...
    # Compute the mean AUROC
    interp_tpr_estimate = np.mean(interp_precisions, axis = 0)
    ...
    # Confidence intervals on sensitivity (tpr)
    lower_ci_tpr = np.percentile(interp_precisions, lower_percentile, axis = 0)
    upper_ci_tpr = np.percentile(interp_precisions, upper_percentile, axis = 0)
    
    # Compute AUROC confidence intervals
    lower_ci = np.maximum(np.percentile(interp_precisions, lower_percentile),0)
    ...
    ax.plot(interp_recalls, interp_tpr_estimate, color="b",label=r"AUCPR: %0.3f" % aucpr_estimate,lw=2,alpha=1,)
    if style == 'science':
        plt.style.use("science")
        ax.fill_between(interp_recalls, lower_ci_tpr, upper_ci_tpr, color="dodgerblue", alpha=0.3, label=r"%d%s CI: [%0.3f, %0.3f]" % (int((1-alpha)*100),'\%', lower_ci, upper_ci))
    else:
        ax.fill_between(interp_recalls, lower_ci_tpr, upper_ci_tpr, color="dodgerblue", alpha=0.3, label=r"%d%%CI: [%0.3f, %0.3f]" % (int((1-alpha)*100), lower_ci, upper_ci))
    ax.set(
        xlim=[-0.02, 1.02],
        ylim=[-0.02, 1.02],
    )
